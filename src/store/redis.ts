...
/**
 * Creates a Redis-backed CommentStore using sorted sets.
 ...
 */
export function createRedisStore(client: RedisLike): CommentStore {
  return {
    ...
    async cleanup(olderThan) {
      ...
      await Promise.all(
        keys.map((key) => client.zremrangebyscore(key, "-inf", olderThan)),
      );
    },
  };
}
